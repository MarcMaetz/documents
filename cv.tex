\documentclass[a4paper,11pt]{article}

\usepackage[margin=1.8cm]{geometry}
\usepackage{tikz}
\usetikzlibrary{shadows}
\usepackage[hidelinks]{hyperref}
\usepackage{enumitem}
\usepackage{amssymb}
\pagestyle{empty}
\usepackage[default]{lato}


\setlength{\parindent}{0pt}
\setlist[itemize]{leftmargin=8pt, itemsep=1pt, topsep=1pt, labelsep=3pt, label=\raisebox{0.25ex}{\scalebox{0.7}{\textbullet}}}

\definecolor{accent}{HTML}{1F4E79}
\definecolor{textgray}{gray}{0.25}
\definecolor{boxyellow}{HTML}{FFF176}

% Star rating command
\newcommand{\starrating}[1]{%
  \ifnum#1=3 $\bigstar$ $\bigstar$ $\bigstar$\fi
  \ifnum#1=2 $\bigstar$ $\bigstar$ $\star$\fi
}

% Title box: yellow border, no fill, black text, full column width
\newcommand{\boxedtitle}[1]{%
  \tikz[baseline=(t.base)]{
    \node[
      draw=boxyellow,
      fill=white,   % was: fill=none
      text=black,
      line width=2pt,
      inner xsep=6pt,
      inner ysep=4pt,
      rounded corners=2pt,
      text width=\linewidth,
      align=left,
      drop shadow={shadow xshift=1pt, shadow yshift=0pt, fill=black!15},
      drop shadow={shadow xshift=0.707pt, shadow yshift=0.707pt, fill=black!15},
      drop shadow={shadow xshift=0pt, shadow yshift=1pt, fill=black!15},
      drop shadow={shadow xshift=-0.707pt, shadow yshift=0.707pt, fill=black!15},
      drop shadow={shadow xshift=-1pt, shadow yshift=0pt, fill=black!15},
      drop shadow={shadow xshift=-0.707pt, shadow yshift=-0.707pt, fill=black!15},
      drop shadow={shadow xshift=0pt, shadow yshift=-1pt, fill=black!15},
      drop shadow={shadow xshift=0.707pt, shadow yshift=-0.707pt, fill=black!15},
      drop shadow={shadow xshift=2pt, shadow yshift=-2pt, fill=black!10},
      drop shadow={shadow xshift=3pt, shadow yshift=-3pt, fill=black!5}
    ] (t) {\bfseries #1};
  }%
}

% Content box: black border, no fill, gray text, auto height
\newcommand{\boxedcontent}[1]{%
  \tikz[baseline=(c.base)]{
    \node[
      draw=white,
      fill=white,   % was: fill=none
      text=textgray,
      line width=1pt,
      inner xsep=6pt,
      inner ysep=6pt,
      rounded corners=2pt,
      text width=\linewidth,
      align=left,
      drop shadow={shadow xshift=1pt, shadow yshift=0pt, fill=black!15},
      drop shadow={shadow xshift=0.707pt, shadow yshift=0.707pt, fill=black!15},
      drop shadow={shadow xshift=0pt, shadow yshift=1pt, fill=black!15},
      drop shadow={shadow xshift=-0.707pt, shadow yshift=0.707pt, fill=black!15},
      drop shadow={shadow xshift=-1pt, shadow yshift=0pt, fill=black!15},
      drop shadow={shadow xshift=-0.707pt, shadow yshift=-0.707pt, fill=black!15},
      drop shadow={shadow xshift=0pt, shadow yshift=-1pt, fill=black!15},
      drop shadow={shadow xshift=0.707pt, shadow yshift=-0.707pt, fill=black!15},
      drop shadow={shadow xshift=2pt, shadow yshift=-2pt, fill=black!10},
      drop shadow={shadow xshift=3pt, shadow yshift=-3pt, fill=black!5}
    ] (c) {#1};
  }%
}

\begin{document}

% --- Right sidebar background with TikZ (only decoration) ---
\begin{tikzpicture}[remember picture,overlay]
  \fill[accent!5]
    (current page.north east)
    rectangle ++(-0.385\paperwidth,-\paperheight);
\end{tikzpicture}

% --- Header (normal LaTeX) ---
{\LARGE\bfseries Marc Maetz}\\[2pt]
{\large Software Engineer}

\vspace{0.8cm}

% --- Two-column layout with minipages ---
\noindent
\begin{minipage}[t]{0.60\textwidth}
  \raggedright

  \boxedtitle{Skills}\\[2pt]
  \boxedcontent{%
    \begin{minipage}[t]{0.48\textwidth}
      \raggedright
      NodeJS, TypeScript, Express \starrating{3}\\
      C, C\texttt{++}, Java, Python, R \starrating{3}\\
      Google Cloud Platform (GCP) \starrating{3}\\
      Microservices, APIs, Databases \starrating{3}\\
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
      \raggedright
      Performance optimisation \starrating{2}\\
      Complex algorithms \starrating{2}\\
      Linux \starrating{2}\\
      High-performance computing \starrating{2}\\
    \end{minipage}
  }

  \vspace{0.7cm}

  \boxedtitle{Experience}\\[2pt]
  \boxedcontent{%
    \textbf{Backend Software Engineer (TypeScript, Python)}\\
    Yokoy \textbar\ Finance \hfill 2022--now, Zurich (CH)\\[-1pt]
    \begin{itemize}
      \item Scaled a microservice backend for a smart card payment platform by an order of magnitude.
      \item Developed internal tooling to speed up incident investigation and resolution.
      \item Worked in a fast-paced agile team with cross-functional collaboration.
      \item Took ownership of complex problems from initial framing to solution.
    \end{itemize}
  }

  \vspace{0.3cm}

  \boxedcontent{%
    \textbf{Software Engineer (C)}\\
    AutoForm \textbar\ Mechanical engineering \hfill 2018--2022, Zurich (CH)\\[-1pt]
    \begin{itemize}
      \item Implemented performance-critical algorithms for sheet metal forming simulations.
      \item Focused on numerical robustness and computational efficiency.
    \end{itemize}
  }

  \vspace{0.3cm}

  \boxedcontent{%
    \textbf{Software Developer (Java)}\\
    Unitek Engineering \textbar\ Finance \hfill 2017--2018, Zurich (CH)\\[-1pt]
    \begin{itemize}
      \item Built adapters to integrate and expose external fund information.
    \end{itemize}
  }

  \vspace{0.3cm}

  \boxedcontent{%
    \textbf{R\&D Intern (MATLAB)}\\
    KIER \textbar\ Mechanical engineering \hfill 2014, Daejeon (KR)\\[-1pt]
    \begin{itemize}
      \item Contributed to simulation and analysis for solar concentrator research.
    \end{itemize}
  }

\end{minipage}%
\hfill
\begin{minipage}[t]{0.35\textwidth}
  \raggedright

  \boxedtitle{Contact}\\[2pt]
  \boxedcontent{%
    marc.maetz@proton.me\\
    +41 77 434 15 25\\
    Zurich, Switzerland\\
    Swiss and French citizen\\
    \href{https://linkedin.com/in/marc-maetz}{linkedin.com/in/marc-maetz}
  }

  \vspace{0.7cm}

  \boxedtitle{Languages}\\[2pt]
  \boxedcontent{%
    German (native)\\
    French (native)\\
    English (fluent)
  }

  \vspace{0.7cm}

  \boxedtitle{Education}\\[2pt]
  \boxedcontent{%
    ETH Zurich, Master of Science\\
    Computational Science and \\Engineering
  } \\ \vspace{0.1cm}

    \boxedcontent{%
    ETH Zurich, 
    Bachelor of Science\\
    Physics
  }
  \\
  \vspace{0.1cm}
    \boxedcontent{%
    KTH Royal Institute of Technology, Stockholm\\
    Exchange semester
  }

  \vspace{0.7cm}

  \boxedtitle{Beyond work}\\[2pt]
  \boxedcontent{%
    Amateur theatre\\
    Running, cycling\\
    Gym\\
    Climbing, mountaineering
  }

\end{minipage}

\end{document}

